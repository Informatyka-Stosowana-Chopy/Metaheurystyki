...
class Algorithm:
    ...
    def __read_attraction(self):
        ...
        for index, row in data.iterrows():
            self.all_attraction.append(
                Attraction(int(row['CUST_NO']), float(row['XCOORD']), float(row['YCOORD']), float(row['DEMAND']),
                           float(row['READY_TIME']), float(row['DUE_DATE']), float(row['SERVICE_TIME'])))
        ...
        for attraction_x in self.all_attraction:
            ...
            for attraction_y in self.all_attraction:
                help_array.append(
                    math.sqrt(pow(attraction_x.x - attraction_y.x, 2) + pow(attraction_y.y - attraction_x.y, 2)))
